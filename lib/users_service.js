...
service.create = function(emailParam,passwordParam,fullnameParam,addressParam,callback){
  var email = helpers.getValidEmailStringOrFalse(emailParam);
  var password = helpers.getValidStringOrFalse(passwordParam, 0);
  var fullname = helpers.getValidStringOrFalse(fullnameParam, 0);
  var address = helpers.getValidStringOrFalse(addressParam, 0);
                  ...
                  if (userCartId){
                    ...
                    _data.delete('carts',userCartId,function(err){
                        if (!err) {
                          ...
                          callback(500, errors.ErrCreate("The user deleted, but errors were encountered while attempting to delete the user's cart."));
                        }
                      });
                  } else {
                    helpers.debug_ok(debugModule, errors.OkCreate("The user deleted"));
